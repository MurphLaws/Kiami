...
function Home() {
	...
	return (
		<HomeContent user={user} signInUrl={signInUrl} signUpUrl={signUpUrl} />
	);
}
...
function Content() {
	const { data: leads } = useSuspenseQuery(convexQuery(api.leads.list));

	return (
		<div className="flex flex-col gap-8 max-w-lg mx-auto">
			<p>Welcome {leads.length} leads!</p>
			<p>
				Click the button below and open this page in another window - this data
				is persisted in the Convex cloud database!
			</p>
			<p>
				Leads: {leads.length === 0 ? "Click the button!" : leads.join(", ")}
			</p>
			<p>
				Edit{" "}
